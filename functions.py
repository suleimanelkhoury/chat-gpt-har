...
def algorithm_recommendation(decription,k):
    return f"""
                        ...
                        """
def data_exploration(description,head,k):
    return f"""
                        in a Human Activity Recognition Context, Given the following description for a dataset delimited by ''':
                        ''' 
                        {description} 
                        '''
                        and the first {k} rows of the segment s01 performed by the subject p01 for the activity a01 delimited by < >:
                        <
                        {head}
                        >
...
def descriptive_statistics(description,head,k):
    return f"""
                        in a Human Activity Recognition Context, Given the following description for a dataset delimited by ''':
                        ''' 
                        {description} 
...
def data_exploration_pipeline(history,description,sample,sample_description,k):
    ...
    for i in range(len(values)):
        ...
        logging.info(output[output.rfind("```python")+9:output.rfind("```")])
        message = message + output[output.rfind("```python")+9:output.rfind("```")] +"\n"
