...
def main():
    import gradio as gr
    if gr.__version__ not in ['3.28.3','3.32.2']: assert False, "需要特殊依赖，请务必用 pip install -r requirements_old.txt 指令安装依赖，详情信息见requirements.txt"
    ...
    with gr.Blocks(title="ChatGPT Academic Optimization", theme=set_theme, analytics_enabled=False, css=advanced_css) as demo:
        ...
        with gr_L1():
            ...
            with gr_L2(scale=1):
                ...
                with gr.Accordion("Change Model & SysPrompt & Interface Layout", open=(LAYOUT == "TOP-DOWN")):
                    ...
                    checkboxes = gr.CheckboxGroup(["basic functional area", "function plug-in area", "bottom input area", "input clear key", "plug-in parameter area"], value=["基础功能区", "函数插件区"], label="Show/Hide Ribbon")
        ...
        def on_dropdown_changed(k):
            ...
            if crazy_fns[k].get("AdvancedArgs", False): # Whether to invoke the advanced plug-in parameter area
                ret.update({plugin_advanced_arg: gr.update(visible=True,  label=f"Advanced parameter description of plugin [{k}]: " + crazy_fns[k].get("ArgsReminder", [f"没有提供高级参数功能说明"]))})
